...
fn fragment(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    var vignette = smoothstep(1.15, 0.15, length(arena_uv));
    if (material.arena_size.z > 0.5) {
        let radius = max(min(material.arena_size.x, material.arena_size.y) * 0.5, 1.0);
        vignette = smoothstep(1.08, 0.16, length(world_pos / radius));
    }
