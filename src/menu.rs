...
fn spawn_settings_column(parent: &mut ChildSpawnerCommands, font: Handle<Font>, config: &SimConfig) {
    parent
        .spawn((
            Node {
                width: px(610),
        ...
        .with_children(|column| {
            ...
            spawn_setting_row(column, font.clone(), "Клетки", MenuTextValue::Cells, config.cells);
            spawn_setting_row(column, font.clone(), "Еда", MenuTextValue::Food, config.food);
            spawn_setting_row(
                column,
                font.clone(),
                "Ширина арены",
            ...
            column
                ...
                .with_children(|advanced| {
                    spawn_setting_row(advanced, font.clone(), "Seed", MenuTextValue::Seed, config.seed as usize);
...
fn spawn_launch_column(parent: &mut ChildSpawnerCommands, font: Handle<Font>, config: &SimConfig) {
    parent
        ...
        .with_children(|column| {
            ...
            spawn_preset_button(column, font.clone(), MenuPreset::Balanced, "Баланс", "10 000 клеток, 2 000 еды");
            spawn_preset_button(column, font.clone(), MenuPreset::Performance, "Производительность", "5 000 клеток, меньше объектов");
            spawn_preset_button(column, font.clone(), MenuPreset::Stress, "Стресс-тест", "20 000 клеток, много еды");

            spawn_section_title(column, font.clone(), "Сводка запуска");
            spawn_summary_row(column, font.clone(), "Клетки", MenuTextValue::Cells, config.cells.to_string());
            spawn_summary_row(column, font.clone(), "Еда", MenuTextValue::Food, config.food.to_string());
            spawn_summary_row(
                column,
                font.clone(),
                "Арена",
            ...
            spawn_summary_row(column, font.clone(), "VSync", MenuTextValue::Vsync, vsync_label(config.vsync));
...
fn menu_interaction_system(
    ...
    for (entity, interaction, input_field, start_btn, preset, section_toggle, vsync_toggle) in
...
fn keyboard_input_system(
    ...
    **text = current_str.clone();
    ...
    match input_field.value_type {
        ...
        MenuTextValue::Vsync | MenuTextValue::Arena => {}
    }
}
